...
# Delete existing weekly task if it exists
$oldTask = Get-ScheduledTask -TaskName "EastboundWeeklyAnalysis" -ErrorAction SilentlyContinue
if ($oldTask) {
    Write-Host "Removing old weekly task..."
    Unregister-ScheduledTask -TaskName "EastboundWeeklyAnalysis" -Confirm:$false
}
